...
const APPS = [
  ...
  { name: "Kling AI", logo: <KlingAI /> },
] as const;

function IntegrationsRow(props: {
  reverse?: boolean;
  className?: string;
  apps: readonly (typeof APPS)[number][];
  duration?: number;
}) {
  const orderedApps = props.reverse ? [...props.apps].reverse() : props.apps;
  return (
    <div className={props.className}>
      <motion.div
        animate={{ translateX: props.reverse ? "50%" : "-50%" }}
        transition={{
        ...
        {[
          ...new Array(2).fill(0).map((_, index) => (
            <React.Fragment key={index}>
              {orderedApps.map(({ name, logo }) => (
                <Card
                  key={`${index}-${name}`}
                  className="aspect-video w-40 shrink-0 shadow-none"
                >
                  <CardContent className="flex flex-col items-center justify-center gap-3">
                    <div className="shrink-0 *:size-12 [&_svg]:size-12">
                      {logo}
                    </div>
...
export default function IntegrationsHorizontal() {
  return (
    <section className="bg-background py-20">
      <div className="mx-auto max-w-6xl px-4 md:px-6">
        <div className="mb-8 flex items-end justify-between">
          <div className="max-w-2xl space-y-3">
            ...
            <p className="text-muted-foreground leading-relaxed">
              Automate and personalize your forms. Connect existing tools from
        ...
        <div className="flex flex-col gap-4 overflow-hidden mask-[linear-gradient(to_right,transparent,black_15%,black_85%,transparent)]">
          <IntegrationsRow apps={APPS} duration={28} />
          <IntegrationsRow apps={APPS} reverse duration={32} />
        </div>
      </div>
    </section>
  );
