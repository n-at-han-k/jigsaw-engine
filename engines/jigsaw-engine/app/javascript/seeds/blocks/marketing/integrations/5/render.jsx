  ...
  return (
    ...
    >
      <motion.div
        ...
        style={props.reverse ? { marginBottom: "140px" } : undefined}
      >
        {[
          ...new Array(2).fill(0).map((_, index) => (
            <React.Fragment key={index}>
              {orderedApps.map(({ name, logo }) => (
                <Card key={`${index}-${name}`} className="w-40 shadow-none">
    ...
    <section className="bg-background py-20">
      <div className="mx-auto max-w-7xl px-4 md:px-6">
        <div className="grid gap-12 lg:grid-cols-[1fr_1.2fr] lg:items-center">
          <div className="mx-auto flex max-w-2xl flex-col justify-center space-y-4">
            <Badge variant="secondary">Integrations</Badge>
            <h2 className="text-4xl font-bold leading-tight lg:text-4xl">
              Integrate Bundui with your favourite apps
            </h2>
          ...
          <div className="flex max-h-[440px] justify-center gap-4 overflow-hidden mask-[linear-gradient(to_bottom,transparent,black_25%,black_75%,transparent)]">
            <IntegrationsColumn apps={APPS} />
            <IntegrationsColumn apps={APPS} reverse />
          </div>
        </div>
      </div>
    </section>
